...
def efficient_generalized_steps(x, seq, model, b, H_funcs, y_0, sigma_0, etaB, etaA, etaC, config, cls_fn=None, classes=None):
    with torch.no_grad():
        ...
        for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), total=len(seq), desc='efficient_generalized_steps'):
            ...
            if cls_fn == None:
                if config.model.type == 'mdt':
                    ...
                    #diffusion = create_diffusion(str(config.model.diffusion_timesteps), diffusion_steps=config.model.diffusion_timesteps)
